	...
	*/
	do {
		...
		if (cx > 0) {
			...
			if (drawoct & 4)  points.push_back(SDL_Point{xmcx, ypcy}); // result |= pixel(renderer, xmcx, ypcy);
			if (drawoct & 2)  points.push_back(SDL_Point{xpcx, ypcy}); // result |= pixel(renderer, xpcx, ypcy);
			if (drawoct & 32) points.push_back(SDL_Point{xmcx, ymcy}); // result |= pixel(renderer, xmcx, ymcy);
			if (drawoct & 64) points.push_back(SDL_Point{xpcx, ymcy}); // result |= pixel(renderer, xpcx, ymcy);
		} else {
			if (drawoct & 96) points.push_back(SDL_Point{x, ymcy}); // result |= pixel(renderer, x, ymcy);
			if (drawoct & 6)  points.push_back(SDL_Point{x, ypcy}); // result |= pixel(renderer, x, ypcy);
		}
		...
		if (cx > 0 && cx != cy) {
			ypcx = y + cx;
			ymcx = y - cx;
			if (drawoct & 8)   points.push_back(SDL_Point{xmcy, ypcx}); // result |= pixel(renderer, xmcy, ypcx);
			if (drawoct & 1)   points.push_back(SDL_Point{xpcy, ypcx}); // result |= pixel(renderer, xpcy, ypcx);
			if (drawoct & 16)  points.push_back(SDL_Point{xmcy, ymcx}); // result |= pixel(renderer, xmcy, ymcx);
			if (drawoct & 128) points.push_back(SDL_Point{xpcy, ymcx}); // result |= pixel(renderer, xpcy, ymcx);
		} else if (cx == 0) {
			if (drawoct & 24)  points.push_back(SDL_Point{xmcy, y}); // result |= pixel(renderer, xmcy, y);
			if (drawoct & 129) points.push_back(SDL_Point{xpcy, y}); // result |= pixel(renderer, xpcy, y);
		}
...
//					result |= pixel(renderer, xph, ymk);
                    ...
                    points.push_back( SDL_Point{xmh, ypk} );
                    points.push_back( SDL_Point{xph, ypk} );
                    points.push_back( SDL_Point{xmh, ymk} );
                    points.push_back( SDL_Point{xph, ymk} );

				} else {
//					result |= pixel(renderer, xmh, y);
//					result |= pixel(renderer, xph, y);

                    points.push_back( SDL_Point{xmh, y} );
                    points.push_back( SDL_Point{xmh, y} );
				}
				ok = k;
				xpi = x + i;
...
//					result |= pixel(renderer, xpi, ymj);
                    ...
                    points.push_back( SDL_Point{xmi, ypj} );
                    points.push_back( SDL_Point{xpi, ypj} );
                    points.push_back( SDL_Point{xmi, ymj} );
                    points.push_back( SDL_Point{xpi, ymj} );
				} else {
//					result |= pixel(renderer, xmi, y);
//					result |= pixel(renderer, xpi, y);

                    points.push_back( SDL_Point{xmi, y} );
                    points.push_back( SDL_Point{xpi, y} );
				}
				oj = j;
			}
...
//					result |= pixel(renderer, xpj, ymi);
                    ...
                    points.push_back( SDL_Point{xmj, ypi} );
                    points.push_back( SDL_Point{xpj, ypi} );
                    points.push_back( SDL_Point{xmj, ymi} );
                    points.push_back( SDL_Point{xpj, ymi} );

				} else {
//					result |= pixel(renderer, xmj, y);
//					result |= pixel(renderer, xpj, y);

                    points.push_back( SDL_Point{xmj, y} );
                    points.push_back( SDL_Point{xpj, y} );
				}
				oi = i;
				xmk = x - k;
				xpk = x + k;
...
//					result |= pixel(renderer, xpk, ymh);
                    ...
                    points.push_back( SDL_Point{xmk, yph} );
                    points.push_back( SDL_Point{xpk, yph} );
                    points.push_back( SDL_Point{xmk, ymh} );
                    points.push_back( SDL_Point{xpk, ymh} );

				} else {
//					result |= pixel(renderer, xmk, y);
//					result |= pixel(renderer, xpk, y);

                    points.push_back( SDL_Point{xmk, y} );
                    points.push_back( SDL_Point{xpk, y} );
				}
				oh = h;
			}
